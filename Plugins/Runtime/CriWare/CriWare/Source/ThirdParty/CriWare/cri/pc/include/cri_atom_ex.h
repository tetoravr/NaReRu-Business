...
/* バージョン情報 */
/* Version Number */
#define CRIATOMEX_VERSION				(0x02200000)
#define CRIATOMEX_VER_NUM				"2.20.00"
 ...
 * （ ::CriAtomExStreamingCacheUnitConfig ）に対し、デフォルト値をセットします。<br>
...
typedef struct CriAtomExConfigTag {
	...
	*/
	...
	/*JP
		...
		::criAtomEx_SetDefaultConfig マクロにより、cri_atom.hヘッダーに定義されているバージョン番号が設定されます。<br>
		\attention
 ...
 * ::criAtomExDecrypter_Create 関数の引数として指定します。<br>
 * \attention
 ...
 * ::criAtomExDecrypter_Create 関数の戻り値として返されます。<br>
 * \sa criAtomExDecrypter_Create
 */
 ...
 * ::criAtomExAcf_GetSelectorInfo 関数に引数として渡します。<br>
 * \attention
 ...
 * \brief ADX1暗号解除キーの設定
 * \ingroup ATOMEXLIB_GLOBAL
 * \param[in]	key_string		ADX1暗号解除キー文字列
 * \par 説明:
 * ADX1向けに作成された、暗号化ADXデータの復号に使用するキー文字列を指定します。<br>
 * \par 備考:
 * 本関数は、ADX1のADXT_SetDefKeyString 関数に相当する機能です。<br>
 * \attention:
 * 本関数は、ADX1用に作成された暗号化ADXデータを、
 * ADX2ランタイム上で再生するための機能です。<br>
 * ADX2 SDK付属のツール（Atom CraftやAtom Encoder等）で作成された音声データを復号するには、
 * 本関数ではなく、デクリプタAPI（::criAtomExDecrypter_Create 関数等）を使用する必要があります。<br>
 * <br>
 * 本関数で設定された暗号解除キーは、
 * 暗号化されていないADXデータをデコードする際には使用されません。<br>
 * （ADX1を使用するケースと動作が異なります。）<br>
 * 暗号化されていないADXデータをノイズ化したい場合には、
 * 本関数とデクリプタAPIを併用する必要があります。<br>
 * <br>
 * 本関数はリリース済みのタイトルのデータを再利用するためだけに用意されています。<br>
 * 新規にADXデータを作成する際には、ADX2 SDK付属のツールを使用して暗号化を行ってください。<br>
 * \sa criAtomExDecrypter_Create
 */
 ...
 * \par 説明:
 * DSPバス設定からDSPバスを構築するのに必要なワーク領域サイズを計算します。<br>
 * ::criAtomExAsr_CalculateWorkSizeForDspBusSetting 関数と違い、
...
/*JP
 * \brief トラックトランジションバイセレクターコールコールバック関数の登録
 * \ingroup ATOMEXLIB_GLOBAL
 ...
 * \ingroup ATOMEXLIB_DECRYPTER
 * \param[in]	p_config	デクリプタ作成用コンフィグ構造体
 * \return		CriSint32	ワーク領域サイズ
 * \retval		0以上		正常に処理が完了
 * \retval		-1			エラーが発生
 * \par 説明:
 ...
 * ::criAtomEx_SetUserAllocator マクロによるアロケーター登録を行わずに
 * ::criAtomExDecrypter_Create 関数でデクリプタを作成する場合、
 * 本関数で計算したサイズ分のメモリをワーク領域として渡す必要があります。<br>
 * <br>
 * ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。<br>
 * ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。<br>
 ...
 * \ingroup ATOMEXLIB_DECRYPTER
 * \param[in]	p_config				デクリプタ作成用コンフィグ構造体
 * \param[in]	p_work					ワーク領域
 * \param[in]	work_size				ワーク領域サイズ
 * \return		CriAtomExDecrypterHn	デクリプタハンドル
 * \par 説明:
 ...
 * （暗号化されていない音声データや、暗号キーの異なる音声データは全てノイズとなります。）<br>
 * <br>
 * 引数 config の情報は、関数内でのみ参照されます。<br>
 * 関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても
 * 問題ありません。
 ...
 * <br>
 * デクリプタ作成後にボイスプールを作成した場合は、
 * ボイスプールを破棄するまでデクリプタを破棄しないでください。<br>
 * ボイスプールを残した状態でデクリプタを破棄した場合、
 * 音声データが正しく再生されなくなったり、アクセス違反が発生する恐れがあります。<br>
 * <br>
 * 本関数を実行後、必ず対になる ::criAtomExDecrypter_Destroy 関数を実行してください。<br>
 * また、 ::criAtomExDecrypter_Destroy 関数を実行するまでは、
 ...
 * また、DSP FXタイプはASRを使用しない環境（プラットフォーム固有のFXを使用する場合）もしくは、<br>
 * ACF Ver.1.15.01 以前のACFを使用した時に取得可能です。<br>
 ...
 * \param[in]	nonce		ノンス
 * \par 説明:
 * ACBデータの暗号を解除するための復号キーを指定します。<br>
 * ::criAtomExAcb_LoadAcbFile 関数等でACBファイルをロードした後に本関数を実行することで、
 * ACBハンドルごとに異なる復号キーを使用することが可能です。<br>
 * （デクリプタによる設定を無視して、当該ACBファイルについては本関数で指定した復号キーが使用されます。）
 * \par 備考:
 * ノンスは将来の為に予約されています。<br>
 * 現状は必ず0を指定してください。<br>
 * \attention
 * 波形データを含まないACBファイルに対し、後から ::criAtomExAcb_AttachAwbFile 
 * 関数でAWBファイルをアタッチする場合、初回AWBファイルアタッチ後に本関数を実行する必要があります。<br>
 * （暗号化の方式によっては、AWBファイルから復号に必要な情報を取得するケースがあるため。）
 */
void CRIAPI criAtomExAcb_Decrypt(
	CriAtomExAcbHn acb_hn, CriUint64 key, CriUint64 nonce);
 ...
 * チャンクの並び順がRIFFチャンク、fmtチャンク、dataチャンクではない場合や、
 ...
 * ::CriAtomExConfigのenable_category_override_by_ex_playerにCRI_TRUEを設定してください。
 ...
 * プレーヤーに設定したラベル情報の個別削除は ::criAtomExPlayer_ClearSelectorLabels 関数を実行してください。<br>
...
/*JP
 * \brief 指定したCue（ID指定）のストリーム用データをキャッシュにロード開始
 * \ingroup ATOMEXLIB_STREAMING_CACHE
 * \param[in]	stm_cache_id	対象とするストリーミングキャッシュID
 * \param[in]	acb_hn			Cueを含んでいるACBハンドル
 * \param[in]	cue_id			キャッシュ対象のCueID
 * \return		CriFsLoaderHn	CriFsLoaderハンドル
 * \retval	NULL		= ロード開始の失敗
 * \retval	NULL以外	= ロード開始の成功
 * \par 説明:
 * ストリーミングキャッシュに対し、IDで指定したCueのストリーミング用データのロードを開始します。<br>
 * 本関数に成功するとロードを開始し、::CRIFSLAODER_STATUS_LOADING状態の
 * ::CriFsLoaderハンドルを返します。<br>
 * 本関数に失敗すると、NULLを返します。<br>
 * 戻り値のCriFsLoaderハンドルが::CRIFSLOADER_STATUS_COMPLETE状態になると、
 * ロードおよびキャッシュ処理は完了です。<br>
 * ロード完了後は、本関数で取得したCriFsLoaderハンドルを
 * ::criFsLoader_Destroy 関数で破棄してください。<br>
 * \par 備考:
 * 本関数は即時復帰です。
 * \attention
 * Cueが複数のストリーム用データを持つ場合、
...
/*JP
 * \brief 指定したCue（名前指定）のストリーム用データをキャッシュにロード開始
 * \ingroup ATOMEXLIB_STREAMING_CACHE
 * \param[in]	stm_cache_id	対象とするストリーミングキャッシュID
 * \param[in]	acb_hn			Cueを含んでいるACBハンドル
 ...
 * \retval	NULL以外	= ロード開始の成功
 * \par 説明:
 * ストリーミングキャッシュに対し、名前で指定したCueのストリーミング用データをロードします。<br>
 * 本関数に成功するとロードを開始し、::CRIFSLAODER_STATUS_LOADING状態の
 * ::CriFsLoaderハンドルを返します。<br>
 * 本関数に失敗すると、NULLを返します。<br>
 * 戻り値のCriFsLoaderハンドルが::CRIFSLOADER_STATUS_COMPLETE状態になると、
