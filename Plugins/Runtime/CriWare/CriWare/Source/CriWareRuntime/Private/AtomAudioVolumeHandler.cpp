		...
		// リスナーがAudioVolumeに入っていない場合で、サウンドがAudioVolumeに入っていた場合はそのボリュームの外向けのバスセンド量を適用する
		{
			...
			if (IsExistingSoundAudioVolume_Bus && AtomAudioVolumeParam && AtomAudioVolumeParam->BusSendInterpolateSettings.Num() > 0) {
				...
				for (auto& sound_cue_bus : AtomAudioVolumeParam->BusSendInterpolateSettings) {
					BusInfo->ValueToApplyMap.Add(sound_cue_bus.BusName, sound_cue_bus.BusSendLevelForOutside);
				}
				BusInfo->bIsInAudioVolumeAtFirstPlay = true;
			}
		}
		...
		// リスナーがAudioVolumeに入っていない場合で、サウンドがAudioVolumeに入っていた場合、そのボリュームの外向けのAisac値を適用する
		{
			...
			if (IsExistingSoundAudioVolume_Aisac && AtomAudioVolumeParam_Aisac && AtomAudioVolumeParam_Aisac->AisacControlInterpolateSettings.Num() > 0) {
				...
				for (auto& aisac_control : AtomAudioVolumeParam_Aisac->AisacControlInterpolateSettings) {
					AisacInfo->ValueToApplyMap.Add(aisac_control.AisacControlName, aisac_control.AisacControlValueForOutside);
				}
				AisacInfo->bIsInAudioVolumeAtFirstPlay = true;
			}
		}
...
void FAtomAudioVolumeHandler::FirstProcessAudioVolume(bool bIsEnableAudioVolume, bool bIsUseDistanceForEnablingAudioVolume, float DistanceForEnablingAudioVolumeFlag)
{
	...
	/* BusSend系下準備 */
	/* DspBusSettingsのBusでAudioVolume用のBusを用意しておき、Volumeに入る前は0に設定し、入った際にAudioVolumeで設定したBusSendLevelになる */
	BusInfo->SoundAudioVolume = AAtomAudioVolume::GetAudioSettings(SoundLocation, EAtomAudioVolumeType::UseBus);
	bool bIsExistingListenerAudioVolume_Bus = (BusInfo->ListenerAudioVolume.Get() && BusInfo->ListenerAudioVolume.Get()->GetEnabled());
	/* BusSend系下準備終わり */

	/* Aisac系下準備 */
	AisacInfo->SoundAudioVolume = AAtomAudioVolume::GetAudioSettings(SoundLocation, EAtomAudioVolumeType::UseAisacControl);
	bool bIsExistingListenerAudioVolume_Aisac = (AisacInfo->ListenerAudioVolume.Get() && AisacInfo->ListenerAudioVolume.Get()->GetEnabled());
	/* Aisac系下準備終わり */
	...
	float DistanceForEnablingAudioVolumeFlag)
{
	...
	if (isSoundExistingInAudioVolume) {
		...
		auto NeighborAtomAudioVolumeParam_Bus = AAtomAudioVolume::GetAtomAudioVolumeSettingsToApply(BusInfo->SoundNeighbourAudioVolume.Get());
		...
		auto NeighborAtomAudioVolumeParam_Aisac = AAtomAudioVolume::GetAtomAudioVolumeSettingsToApply(AisacInfo->SoundNeighbourAudioVolume.Get());
