	...
	LoadTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&, DistanceFactor, ContentDir]() {
		...
		if (AcbHn) {
			...
			for (int32 i = 0; i < NumCue; i++) {
				...
				LoadCueInfo(i, CueInfo);
				CueInfo.AttenuationDistance.MaxDistance /= DistanceFactor;
				CueInfo.AttenuationDistance.MinDistance /= DistanceFactor;
		...
		// signal game thread for blueprints
...
void USoundAtomCueSheet::LoadCueInfo(int32 CueIndex, FAtomCueInfo& CueInfo)
{
	if (!AcbHn) {
		return;
	}
	...
	CriBool bResult = criAtomExAcb_GetCueInfoByIndex(AcbHn, CueIndex, &AtomExCueInfo);
	if (bResult != CRI_FALSE) {
		...
		CueInfo.AttenuationDistance.MaxDistance = AtomExCueInfo.pos3d_info.max_distance;
		CueInfo.AttenuationDistance.MinDistance = AtomExCueInfo.pos3d_info.min_distance;
	}
...
static void CRIAPI criware_on_acb_released_func(void *obj)
{
	...
	if (Context->Binder != NULL) {
		criFsBinder_Destroy(Context->Binder);
	}
